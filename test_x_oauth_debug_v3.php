...
echo "=== X (Twitter) OAuth Signature Debug v3 ===\n\n";
...
try {
    ...
    // Now test the tweet posting with detailed debugging
    echo "\n--- Testing Tweet Posting with Debug ---\n";
    
    // Add Content-Type header for form data
    $tweetHeaders['Content-Type'] = 'application/x-www-form-urlencoded';
    
    echo "Final headers for tweet posting:\n";
    foreach ($tweetHeaders as $key => $value) {
        if ($key === 'Authorization') {
            echo "  {$key}: " . substr($value, 0, 100) . "...\n";
        } else {
    ...
    echo "\nMaking POST request to: {$tweetUrl}\n";
    echo "Data: " . http_build_query($tweetData) . "\n";
    ...
    if ($tweetResponse['header_code'] == 200) {
        ...
        // Let's also check if there are any response headers that might give us clues
        if (isset($tweetResponse['headers'])) {
            echo "\nResponse Headers:\n";
            foreach ($tweetResponse['headers'] as $header) {
                echo "  {$header}\n";
            }
        }
    }
